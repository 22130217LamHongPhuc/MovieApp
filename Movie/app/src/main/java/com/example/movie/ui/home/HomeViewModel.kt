...
class HomeViewModel @Inject constructor(val repository: MovieRepository) :ViewModel() {
    ...
    fun getDataHomeMovie(){
        viewModelScope.launch {
            // 4.1.3 hệ thống chỉnh _stateHome trạng thái sang đang loading
            _stateHome.update { it.copy(isLoading = true) }

            try {
                coroutineScope {
                   // 4.1.5  Hệ thống thực hiện gọi  phương thức fetchHomeTypeList(type) trong MovieRepository để lấy data phim
